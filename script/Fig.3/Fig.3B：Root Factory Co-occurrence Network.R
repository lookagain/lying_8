...
##
metadata_R <- metadata[metadata$Area_Microbiome =="Root",]
Root_F <- metadata_R[metadata_R$Site =="Factory",]

#
ASV_F <- ASV[,colnames(ASV) %in% Root_F$SampleID]
otutab <- ASV_F[apply(ASV_F, 1, function(x) any(x > 0.005)), ]   
taxon <- taxonomy[rownames(taxonomy) %in% rownames(otutab),]

#
otutab <- as.matrix(otutab)
occor <- rcorr(t(otutab), type = 'spearman')
saveRDS(occor, "Root Factory rcorr list.rds")
...
write_graph(A, "../../figures/Fig.3/Root Factory rcorr.graphml", format="graphml")
