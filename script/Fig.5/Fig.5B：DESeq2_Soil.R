...
metadata_Soil_H <- metadata_Soil[metadata_Soil$Site == "Factory",]
metadata_Soil_V <- metadata_Soil[metadata_Soil$Site == "Village",] 
metadata_Soil <- rbind(metadata_Soil_H,metadata_Soil_V)

ASV <- Table_ASV
ASV <- ASV[,colnames(ASV) %in% rownames(metadata_Soil)]   
...
#Soil Site DESeq2
asv_dataset <- microtable$new(sample_table = metadata_Soil,
                              otu_table = ASV, 
                              tax_table = tax)   #14228 taxa with 0 abundance are removed from the otu_table
...
#Visualization
#biomakers
biomaker_F <- read.csv("../../data/rf_family_S_top30_res-abund-wilcox-sig.csv",check.names = F)
...
hight_phylum <- c("Proteobacteria","Firmicutes","Actinobacteriota", "Bacteroidota","Acidobacteriota","Chloroflexi","Verrucomicrobiota")
...
t1_res_diff_1 <- t1_res_diff[t1_res_diff$hight_phylum %in% "Proteobacteria",]
t1_res_diff_1 <- t1_res_diff_1[order(t1_res_diff_1$num),]
t1_res_diff_2 <- t1_res_diff[t1_res_diff$hight_phylum %in% "Firmicutes",]
t1_res_diff_2 <- t1_res_diff_2[order(t1_res_diff_2$num),]
t1_res_diff_3 <- t1_res_diff[t1_res_diff$hight_phylum %in% "Actinobacteriota",]
t1_res_diff_3 <- t1_res_diff_3[order(t1_res_diff_3$num),]
t1_res_diff_4 <- t1_res_diff[t1_res_diff$hight_phylum %in% "Bacteroidota",]
t1_res_diff_4 <- t1_res_diff_4[order(t1_res_diff_4$num),]
t1_res_diff_5 <- t1_res_diff[t1_res_diff$hight_phylum %in% "Acidobacteriota",]
t1_res_diff_5 <- t1_res_diff_5[order(t1_res_diff_5$num),]
t1_res_diff_6 <- t1_res_diff[t1_res_diff$hight_phylum %in% "Chloroflexi",]
t1_res_diff_6 <- t1_res_diff_6[order(t1_res_diff_6$num),]
t1_res_diff_8 <- t1_res_diff[t1_res_diff$hight_phylum %in% "Verrucomicrobiota",]
t1_res_diff_8 <- t1_res_diff_8[order(t1_res_diff_8$num),]
t1_res_diff_Oth <- t1_res_diff[t1_res_diff$hight_phylum %in% "Others",]
t1_res_diff_Oth <- t1_res_diff_Oth[order(t1_res_diff_Oth$num),]

t1_res_diff_order <- rbind(t1_res_diff_1, t1_res_diff_2, t1_res_diff_3, t1_res_diff_4, t1_res_diff_5, t1_res_diff_6,t1_res_diff_8, t1_res_diff_Oth)
...
P_manhattan_Site_Soil <- 
  ...
  ggtitle(paste("Soil | Village vs Factory")) +
  theme_bw() + 
...
ggsave("../../figures/Fig.5/P_manhattan_Village-vs-Factory_Soil.pdf", P_manhattan_Site_Soil, width = 8, height = 4)
write.csv(t1_res_diff_order,"Soil_Site_DEseq2_sig_result_visual.csv",row.names = F)
